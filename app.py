...
image_slider = pn.widgets.DiscreteSlider(name='Image guidance scale', options=list(np.arange(1,10.5, 0.5)), value=1.5)
guidance_slider = pn.widgets.DiscreteSlider(name='Guidance scale', options=list(np.arange(1,10.5, 0.5)), value=7)
step = pn.widgets.IntSlider(name='Image guidance scale', start=1, end=100, step=1, value=20)
...
interactive_conversation = pn.bind(get_conversations, button_conversation, fi, image_slider, guidance_slider, step)
